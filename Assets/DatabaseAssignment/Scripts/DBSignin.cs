...
public class DBSignin : MonoBehaviour
{
    ...
    private enum SigninError //로그인 오류 발생 시
    {
        ...
        IncorrectPW,  //passwordtmp 값과 DB의 Password값이 동일하지 않는 경우
        ConnectionError //서버 연결 자체의 문제일 경우
    }

    private void ErrorMessage(SigninError error) //각 상황에 따른 오류 메세지
    {
        switch (error)
        {
    ...
    private IEnumerator LoginCoroutine(string id, string password)  //로그인 코루틴
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post(loginUri, loginform))
        {
            ...
            //서버 응답 받을 시
            string response = www.downloadHandler.text;  //서버 응답 받기
